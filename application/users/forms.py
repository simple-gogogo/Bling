...
class RegisterForm(FlaskForm):
    username = StringField('用户名', validators=[DataRequired(message='用户名不能为空'), Length(max=128, message='用户名不能超过128个字')])
    mobile = StringField('手机号', validators=[DataRequired(message='手机号不能为空'), Length(max=32, message='手机号不能超过32位')])
